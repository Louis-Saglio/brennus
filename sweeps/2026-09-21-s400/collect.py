#!/usr/bin/env python3
"""Collect the kiln sweep into results.tsv (all 400 seeds).

Usage: collect.py [jobs.tsv]
Extracts artifacts under artifacts/s<seed>/ for every job in jobs.tsv,
then merges the 26 reused rows from sweeps/2026-09-21-wavrec/results.tsv,
and writes results.tsv next to this script sorted by seed.
...
t=<num>m print; timeouts are flat 45.0.
"""
import json
import re
import subprocess
...
WAVREC = HERE.parent / "2026-09-21-wavrec" / "results.tsv"
...
def main(jobs_tsv):
    ...
    for line in WAVREC.read_text().splitlines()[1:]:
        seed = line.split()[0]
        if seed not in have:
    ...
    e = sum(1 for r in rows if int(r[3]))
    fa = sum(1 for r in rows if int(r[4]))
    nh = sum(1 for r in rows if not int(r[5]))
