...
FEATURES = [
    ...
    ("corral", r"corral at"),
    ("tier3", r"research soldier_(?:attack_melee|attack_ranged|resistance_hack|resistance_pierce)_03"),
    ("stable", r"defense building stable"),
    ("cavbatch", r"training cavalry"),
    ("cavtech", r"research cavalry_"),
]
...
def main(jobs_tsv, outdir):
    ...
    for line in Path(jobs_tsv).read_text().splitlines():
        ...
        counts += [ut.get("Hero", 0), ut.get("Champion", 0), ut.get("Cavalry", 0)]
        pop_limit = 0
        m = re.findall(r"pop=\d+/(\d+)", log)
        if m:
            pop_limit = int(m[-1])
        rows.append([seed, verdict, f"{end_min:.1f}", errs, failed_ai, harness, pop_limit] + counts)
    hdr = ["seed", "verdict", "end_min", "js_errors", "failed_ai", "harness", "pop_limit"] + [n for n, _ in FEATURES] + ["hero", "champ", "cav"]
