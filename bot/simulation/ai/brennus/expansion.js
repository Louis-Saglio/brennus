...
export function ExpansionManager(bot)
{
	this.bot = bot;
	this.expPlan = null; // {spots, next, done, simPct}
	this.expOn = false;
	...
	this.expContested = {};
	// Transient: home-CC land-region pin, stage latches, the relief
	// resource-scan and storehouse-cleanup throttles, the trade-route print
	// latch and the territoryPercent passability cache.
	this.expansionRegion = undefined;
	this.defOn = undefined;
	this.war = undefined;
	this.reliefResourceCheck = undefined;
	this.nextCleanupTurn = undefined;
	this.routeLogged = undefined;
	this.terrPassable = undefined;
}

ExpansionManager.prototype.serialize = function()
{
	return {
...
ExpansionManager.prototype.deserialize = function(data)
{
	this.expPlan = data?.expPlan || null;
	this.expOn = data?.expOn || false;
	this.reliefOn = data?.reliefOn || false;
	this.reliefServedPeak = data?.reliefServedPeak || {};
	this.expContested = data?.expContested || {};
};

// ---------------------------------------------------------------- expansion
ExpansionManager.prototype.expansionOn = function()
{
	if (this.expOn)
		return true;
	if (this.bot.gameState.isResearched("phase_city_generic") &&
		this.bot.gameState.getPopulation() >= 300)
	{
		this.expOn = true;
		const cc = this.bot.getCivicCentre();
		if (cc)
			this.expansionRegion = this.bot.accessibility.getAccessValue(cc.position());
		print(`[HARNESS] t=${(this.bot.gameState.getTimeElapsed() / 60000).toFixed(1)}m expansion stage on (city researched, pop 300)\n`);
	}
	return this.expOn;
};
 ...
 */
ExpansionManager.prototype.defenseOn = function()
{
	if (this.defOn)
		return true;
	...
	{
		this.defOn = true;
		print(`[HARNESS] t=${(this.bot.gameState.getTimeElapsed() / 60000).toFixed(1)}m defense stage on (town researched)\n`);
	}
	return this.defOn;
};
 ...
 * war machine needs.
 */
ExpansionManager.prototype.warOn = function()
{
	if (this.war)
		return true;
	if (this.bot.gameState.isResearched("phase_city_generic"))
	{
		this.war = true;
		print(`[HARNESS] t=${(this.bot.gameState.getTimeElapsed() / 60000).toFixed(1)}m war stage on (city researched)\n`);
	}
	return this.war;
};

ExpansionManager.prototype.expBarterTarget = 52000;

ExpansionManager.prototype.targetTraders = 40;
ExpansionManager.prototype.traderType = "units/{civ}/support_trader";

ExpansionManager.prototype.expMarkets = 2;
 ...
 * food/wood evenly. */
ExpansionManager.prototype.expansionShares = function(total)
{
	if (!total)
	...
	const sites = this.bot.economyManager.dropsiteEdgeList();
	const served = { "stone": 0, "metal": 0 };
	this.bot.economyManager.servedMineIds = new Set();
	for (const res of ["stone", "metal"])
		for (const s of gameState.getResourceSupplies(res).values())
		{
			...
			if (!pos || !s.resourceSupplyAmount() || this.bot.armyManager.nearEnemy(pos, 100, 60))
				continue;
			if (region !== undefined && this.bot.accessibility.getAccessValue(pos) !== region)
				continue;
			if (this.bot.economyManager.edgeDistToSites(pos, sites) > this.bot.mineDistWarn)
				continue;
			...
			if (!this.bot.inOwnTerritory(pos[0], pos[1]))
				continue;
			served[res] += s.resourceSupplyAmount();
			this.bot.economyManager.servedMineIds.add(s.id());
		}
	...
	const woodFrac = this.bot.bankAwareWoodFrac(this.bot.arbiter.books("shares"), 0.5);
...
ExpansionManager.prototype.expansionCandidates = function()
{
	const spots = [];
...
ExpansionManager.prototype.expansionSpotOK = function(spot, halfW, halfD)
{
	...
	const angle = this.bot.placementManager.getPlacementAngle();
	...
	for (let j = tz0; j <= tz1; ++j)
		for (let i = tx0; i <= tx1; ++i)
		{
			...
			if (owner !== this.bot.player && owner !== 0)
				return false;
		}
	return true;
};

ExpansionManager.prototype.expansionSpotNear = function(anchor, halfW, halfD, ccSpots)
{
	for (let r = 0; r <= 60; r += 4)
		for (let a = 0; a < 24; ++a)
		{
			...
			if (this.bot.placementManager.failedSpots.some(f => Math.abs(f[0] - spot[0]) < 6 && Math.abs(f[1] - spot[1]) < 6))
				continue;
			if (ccSpots.some(c => SquareDistance(c, spot) < 200 * 200))
				continue;
			if (this.bot.armyManager.nearEnemy(spot, 100, 60))
				continue;
			if (this.expansionRegion !== undefined &&
				this.bot.accessibility.getAccessValue(spot) !== this.expansionRegion)
				continue;
...
ExpansionManager.prototype.computeExpansionPlan = function()
{
	const gameState = this.bot.gameState;
	...
	for (let j = 0; j < terrH; ++j)
		...
		{
			const idx = i + j * terrW;
			if (costGrid[idx] === 1 && (terr.data[idx] & 0x1F) === this.bot.player)
			{
				own[idx] = 1;
				ownCount++;
			}
	...
	const base = this.bot.getCivicCentre()?.position() || [384, 384];
...
ExpansionManager.prototype.checkReliefExpansion = function()
{
	...
	const stuck = (templateType, latch) =>
		this.bot.placementManager.placeFailSince[templateType] !== undefined &&
		this.bot.turn - this.bot.placementManager.placeFailSince[templateType] >= latch;
	for (const [building, latch] of [["market", 750], ["arsenal", 1200]])
	{
		...
		if (!owned)
		{
			this.reliefFire(`no room: first ${building} placement failing ${((this.bot.turn - this.bot.placementManager.placeFailSince[type]) / 300).toFixed(1)}m`);
			return;
		}
	}
	...
	if (this.bot.turn - (this.reliefResourceCheck || 0) < 150)
		return;
	this.reliefResourceCheck = this.bot.turn;
	const sites = this.bot.economyManager.dropsiteEdgeList();
	for (const res of ["stone", "metal"])
	{
		let supply = 0;
		for (const s of gameState.getResourceSupplies(res).values())
			if (s.position() && s.resourceSupplyAmount() &&
				this.bot.economyManager.edgeDistToSites(s.position(), sites) <= this.bot.mineDistWarn)
...
ExpansionManager.prototype.reliefFire = function(why)
{
	this.reliefOn = true;
	...
	if (cc && this.expansionRegion === undefined)
		this.expansionRegion = this.bot.accessibility.getAccessValue(cc.position());
	print(`[HARNESS] t=${(this.bot.gameState.getTimeElapsed() / 60000).toFixed(1)}m relief expansion on (${why})\n`);
};
 ...
 */
ExpansionManager.prototype.manageStorehouseCleanup = function()
{
	if (this.bot.turn < (this.nextCleanupTurn || 0))
		return;
	this.nextCleanupTurn = this.bot.turn + 150;
	const gameState = this.bot.gameState;
	...
	for (const ent of gameState.getOwnStructures().values())
	{
		...
		if (workers.some(p => SquareDistance(p, pos) < 45 * 45))
			continue;
		if (this.bot.armyManager.nearEnemy(pos, 80, 60))
			continue;
...
ExpansionManager.prototype.manageExpansion = function()
{
	...
	if (!this.expPlan || this.bot.turn - (this.expPlan.turn || 0) >= 750)
		...
		this.expPlan = Object.assign(this.computeExpansionPlan(), { "turn": this.bot.turn });
	const gameState = this.bot.gameState;
	const ccType = gameState.applyCiv("structures/{civ}/civil_centre");
	...
	if (full && !plan.wonderDone)
	{
		...
		else if (!this.bot.buildupManager.willToFightPending(gameState) &&
			!this.bot.constructionManager.pendingBuilds.some(pb => pb.template === wonderType) &&
			!this.bot.arbiter.held("construction"))
		{
			const res = this.bot.arbiter.books("expansion");

			if (res.canAfford({ "wood": 1100, "stone": 1550, "metal": 1100 }))
			{
				const spot = this.findWonderSpot(wonderType);
				if (spot && this.bot.placementManager.placeOrder(wonderType, spot))
				{
					this.bot.arbiter.spend(res, "expansion", { "wood": 1000, "stone": 1500, "metal": 1000 }, "wonder");
	...
	if (full && (plan.marketsPlaced || 0) < this.expMarkets)
	{
		...
		else if (!this.bot.constructionManager.pendingBuilds.some(pb => pb.template === marketType &&
			...
			!this.bot.arbiter.held("construction"))
		{
			...
			if (res.wood >= 600)
			{
				...
				{
					const pos = this.bot.placementManager.findBuildingPosition(marketType, anchor.position(), 20, 80, true, this.expansionRegion);
					if (pos && this.bot.placementManager.placeOrder(marketType, pos))
					{
						this.bot.arbiter.spend(res, "expansion", { "wood": 300 }, "market");
	...
	if (plan.next < plan.spots.length)
	{
		...
		const ccConcurrency = full ? ((this.bot.armyManager.enemyArmy || 0) < 60 ? 3 : 2) : 1;
		const ownCCPos = [];
		for (const ent of gameState.getOwnStructures().values())
		...
		const ccPending = this.bot.constructionManager.pendingBuilds.filter(pb => pb.template === ccType);
		...
		{
			if (this.bot.offenseManager.clearOp && key === this.bot.offenseManager.clearOp.key)
				continue;
			const c = this.expContested[key];
			if (this.bot.armyManager.nearEnemy([c.x, c.z], 100, 60))
				c.seen = this.bot.turn;
			else if (this.bot.turn - c.seen > 150 && (!c.proven || this.bot.turn > c.until))
		...
		while (slots > 0 && plan.next < plan.spots.length && scanned++ < plan.spots.length)
		{
			...
			{
				const c = this.expContested[key];
				if (this.bot.offenseManager.clearOp &&
					Math.abs(c.x - this.bot.offenseManager.clearOp.x) < 100 && Math.abs(c.z - this.bot.offenseManager.clearOp.z) < 100)
					continue;
				const alive = c.proven ? this.bot.turn <= c.until : this.bot.turn - c.seen <= 150;
...

			{
				...
				const enemyNear = this.bot.armyManager.nearEnemy(spot, 100, this.bot.armyManager.enemyArmy > 40 ? 60 : 0);
			...
			const covered = this.bot.offenseManager.clearOp &&
				Math.abs(this.bot.offenseManager.clearOp.x - spot[0]) < 100 && Math.abs(this.bot.offenseManager.clearOp.z - spot[1]) < 100;
			if (!covered && !ownCCPos.some(c => SquareDistance(c, spot) < 260 * 260) &&
				((this.bot.armyManager.enemyArmy || 0) > 100 || this.bot.armyManager.armyCount() < 50))
			{
				plan.spots.splice(plan.next, 1);
				plan.spots.push(spot);
				continue;
			...
			const stoneReserve = this.bot.buildupManager.wonderHoldActive(gameState) ? 1550 : 0;
			if (!res.canAfford({ "wood": 400, "stone": 400 + stoneReserve, "metal": 300 + (stoneReserve ? 1100 : 0) }))
				return;
			if (!this.bot.placementManager.placeOrder(ccType, spot))
				return;
			...
			const party = gameState.getOwnUnits()
				.filter(ent => ent.isGatherer() && ent.position() &&
					!this.bot.armyManager.army[ent.id()] && ent.id() !== this.bot.economyManager.herderId)
				.filterNearest(spot, 6).toEntityArray();
			for (const ent of party)
				ent.construct(ccType, spot[0], spot[1], this.bot.placementManager.getPlacementAngle(), undefined);
			this.bot.arbiter.spend(res, "expansion", { "wood": 300, "stone": 300, "metal": 250 }, "CC");
			slots--;
...
ExpansionManager.prototype.manageTrade = function()
{
	if (!this.expansionOn())
		return;
	const gameState = this.bot.gameState;
	...
	{
		const res = this.bot.arbiter.books("trade");
		if (res.food >= 40000 + 100 && res.metal >= 1200)
		{
			...
			if (gameState.getPopulation() >= gameState.getPopulationLimit())
				for (const ent of gameState.getOwnUnits().values())
					if (ent.isGatherer() && !ent.hasClass("Cavalry") && ent.isIdle() && !this.bot.armyManager.army[ent.id()] &&
			...
			for (const market of markets)
				if ((market.trainingQueue()?.length || 0) <= 1)
				{
					...
					this.bot.arbiter.spend(res, "trade", { "food": 100, "metal": 80 }, "trader");
					break;
				}
		}
	}
...
ExpansionManager.prototype.expansionTechs = [
...
ExpansionManager.prototype.manageExpansionTechs = function()
{
	if (!this.expansionOn())
		return false;
	const gameState = this.bot.gameState;
	const resources = this.bot.arbiter.books("expansionTechs");
	...
	for (const tech of this.expansionTechs)
	{
		...
		if (facility)
		{
			...
			this.bot.arbiter.spend(resources, "expansionTechs", cost, tech);
...
ExpansionManager.prototype.manageExpansionBarter = function(market)
{
	const gameState = this.bot.gameState;
	const res = this.bot.arbiter.books("expansionBarter");
	if (res.stone < this.expBarterTarget || res.metal < this.expBarterTarget)
	{
		...
		if (bestRatio >= 0.35 && res[sell] >= 47000 && this.bot.turn % 15 === 0)
		{
			market.barter(want, sell, 500);
			this.bot.arbiter.spendSell("barter", sell, 500, `barter ${sell}->${want}`);
...
ExpansionManager.prototype.territoryPercent = function()
{
	const terr = this.bot.territoryMap;
	if (!this.terrPassable)
	{
		const pass = this.bot.gameState.getPassabilityMap();
		const mask = this.bot.gameState.getPassabilityClassMask("default-terrain-only");
		const tcell = terr.cellSize / pass.cellSize;
	...
	for (let i = 0; i < this.terrPassable.length; ++i)
	{
		...
		total++;
		const v = terr.data[i];
		if ((v & 0x1F) === this.bot.player && (v & 0x20))
			own++;
	}
...
ExpansionManager.prototype.manageBarter = function()
{
	const gameState = this.bot.gameState;
	...
	if (!market)
		return;
	const res = this.bot.arbiter.books("barter");
	...
	if (!gameState.isResearched("phase_city_generic"))
	{
		...
		if ((res.stone < 750 || res.metal < 750))
		{
			...
			if (res[sell] >= 400)
			{
				...
				this.bot.arbiter.spendSell("barter", sell, 100, `barter ${sell}->${want}`);
		...
		if (res[excess] >= 1300 && (res.wood < 250 || res.food < 200))
		{
			...
			this.bot.arbiter.spendSell("barter", excess, 500, `barter ${excess}->${want}`);
	...
	else
	{
		if (!this.manageExpansionBarter(market))
		{
			...
			// any other food deal.
			if (this.bot.turn % 15 === 0 && res.food >= 4000)
			{
				const willPending = this.bot.buildupManager.willToFightPending(gameState);
				...
				if (want)
				{
					...
					if (prices.sell.food / prices.buy[want] >= 0.5)
					{
						...
						this.bot.arbiter.spendSell("barter", "food", 500, `barter food->${want}`);
			...
			if (res[rich] - res[poor] > 5000 && res[rich] > 6000 && this.bot.turn % 15 === 0)
			{
				const prices = gameState.getBarterPrices();
				if (prices.sell[rich] / prices.buy[poor] >= 0.5)
				{
					market.barter(poor, rich, 500);
					this.bot.arbiter.spendSell("barter", rich, 500, `barter ${rich}->${poor}`);
...
{
	const template = this.bot.gameState.getTemplate(wonderType);
	const halfW = +template.get("Obstruction/Static/@width") / 2 + 0.5;
	const halfD = +template.get("Obstruction/Static/@depth") / 2 + 0.5;
	const angle = this.bot.placementManager.getPlacementAngle();
	const pass = this.bot.gameState.getPassabilityMap();
	...
	for (const anchor of anchors)
		for (let r = 12; r <= anchor[1]; r += 2)
			for (let a = 0; a < 64; ++a)
			{
				...
				if (this.bot.placementManager.failedSpots.some(f => Math.abs(f[0] - x) < 6 && Math.abs(f[1] - z) < 6))
					continue;
				if (this.bot.armyManager.nearEnemy([x, z], 100, 60))
					continue;
				if (!this.bot.placementManager.placementOK(x, z, halfW, halfD, angle, pass, mask, terr))
					continue;
				return [x, z];
			}
	return undefined;
